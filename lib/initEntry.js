// # Entry Module
// This is a module for accessing entry data and functions.
...
// ### *function*: createUser
/**
 * Makes the new user in the database
 *  
 * @param {string} tableName 
 * @param {function} cb -> {}
 */
function createUser(username, name, password, email, location, website, cb){
    ...
    db.run('insert into users values (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [username, name, password, email, location, website,profilepic, profvis, background],
      cb);
}

// ### *function*: createVerUser
/**
 * Makes the new user pending for verification
 *  
 * @param {string} tableName 
 * @param {function} cb -> {}
 */
 ...
 * @param {function} cb -> {error msg or user object}
 */
 ...
 * @param {string} email
 * @param {function} cb -> {error or success message}
 */
 ...
 * 
 * @param {string} code
 * @param {function} cb -> {error or undefined}
 */
 exports.lookupCodeCheck= function(code,cb){
 	db.all('select * from verifyUsers where randVarCode=?', [code], function(err, u){
        ...
        if(user ==undefined){
        	...
        	createUser(user.username, user.name, user.password, user.email, user.location, user.website, cb);
          db.run('insert into isfollowings values (?, ?)',[user.username, user.username],cb);
 ...
 * Note: Another feature can by to check to see if the email is already contained in the database to 
