...
function searchTweets (query) {
	...
	/*db.all(select tweetid from tweets where msg LIKE %query%, function(err, rows) {
	for (var i = 0; i < rows.length; i++) {
		var ntweet = {id: row['tweetid'],
					name: row['name'],
...
//can be deleted if deleteTweet works using sqlite language
...
function deleteTweet(username, tweetID) {
	//var tbd = parseInt(tweetID);
	//db.all(delete from tweets where tweetid = tbd);
	//db.all(delete from hashtags where tweetid = tbd);
	//db.all(delete from mentions where tweetid = tbd);
