...
function getTweetConvoByTweetID(tweetID) {
	var tweetList = [];
	db.get("select convo from tweets where tweetid = ?", [tweetid], function(err, convo) {
		if (convo === null) {
			cb(null);
		} else {
			db.get("select * from tweets where convo = ? order by date desc", [convo], function(err, tweets) {
				cb(tweets);
			});
		}
	/*var myConvo = tweetdb[tweetID].convo;
