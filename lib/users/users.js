...
function checkProfilePermission(loggedin, profileowner) {
	var profperm = userdb[profileowner.uid].profVis;
...
function changeUserProfile(username, nname, nusername, nemail, nlocation, nwebsite, nnewpass, nconfirmnewpass, ncurrentpass) {
	var user = getUserById(username);
	...
	if (ncurrentpass === userdb[user.uid-1].password) {
		...
		if (nusername !== '') { userdb[user.uid-1].username = nusername; }
