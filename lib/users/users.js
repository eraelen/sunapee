...
var usersInTestData = 3; //Used for userid continuation
...
/*
* Gets user's tweets number
* @param username, username
* @return userdb[i].tweets.length, user's tweet number
*/
...
function addUserT(username, id) {
  getUserById(username).tweets.push(id);
	console.log(getUserById(username).tweets);
}
exports.addUserT = addUserT;

// ### *function*: unfollow
/*
* Delete a user from the following list
* @param username, logged in user's username
* @param rmuname, username of the user that is to be removed from the following list
*/
exports.unfollow = function (username, rmuname) {
  console.log("unfollow loggedin "+username+" rmuname "+rmuname);
  var fl = getUserById(username).following;
  console.log(fl);
  fl.splice(fl.indexOf(rmuname), 1); //remove rmuname from fl
  console.log("after deletion "+fl);
  console.log(".following "+getUserById(username).following);
}
...
function getFollowingNum (username) {
	...
	return userdb[uid].following.length;
}
