...
{name: 'Eeyore', username: 'cheerfuldonkey', password: 'eeyore', uid: 4, email: "mynameiseeyore@hundredacrewood.com", location: "Hundred Acre Wood", website: "", profilePic: "/images/users/cheerfuldonkey/eeyore_profilepic.jpg", follower: ["thepooh"], following: ["thepooh"], tweets: [6,7,9,11], profVis: "Public", mentionPerm: "Public", pmPerm: "Public", background: ""},
{name: 'Winnie', username: 'thepooh', password: 'winnie', uid: 5, email: "mynameiswinnie@hundredacrewood.com", location: "Hundred Acre Wood", website: "", profilePic: "/images/users/thepooh/pooh_profilepic.jpg", follower: ["cheerfuldonkey"], following: ["cheerfuldonkey"], tweets: [8,10], profVis: "Public", mentionPerm: "Public", pmPerm: "Public", background: ""}
];
 ...
 */
 exports.lookupRegistrationParams = function(name, username, email, password, retypeP, location, website, cb){
    if(name.length == 0 || username.length == 0 || email.length == 0 || password.length == 0){
        ...
        console.log("Random Var Code:"+randVarCode);
...
function checkProfilePermission(loggedin, profileowner) {
	...
	var profperm = "Public";
	db.all(select uid from userinfo where username = profileowner, function (err, rows) {
		profperm = rows['profvis'];
	};
	if (profperm === "Followers Only") {
		db.all(select username from isfollowing where fusername = loggedin, function (err, rows) {
			if (rows.length != 0) { 
				return true;
			} else {
				return false;
...
function searchPeople (query) {
	...
	/*db.all(select uid from userinfo where username LIKE %query% and name LIKE %query%, function(err, rows) {
	for (var i = 0; i < rows.length; i++) {
		var nuser = {uid: row['uid'],
	...
	if (ncurrentpass === userdb[user.uid-1].password) {
		...
		if (nusername !== '') {  
			...
			for (var i=0; i<followerList.length; i++) {
				var fl = getUserById(followerList[i]).following;
				var idx = fl.indexOf(username);
				fl[idx] = nusername;
			}
			var followingList = getUserById(username).following;
			for (var i=0; i<followingList.length; i++) {
				var fl = getUserById(followingList[i]).follower;
				var idx = fl.indexOf(username);
				fl[idx] = nusername;
			}
			userdb[user.uid-1].username = nusername;
			var mkdirp = require('mkdirp');
			mkdirp('public/images/users/'+nusername, function (err) {
				if (err) console.error(err)
				else console.log('folder created')
			});
		}
