...
exports.checkProfilePermission = function (loggedin, profileowner,cb) {
	...
	var profperm = "Public";
	db.serialize( function() {
		db.get("select profvis from users where username = ?", [profileowner], function (err, p) {
			profperm = p.profVis;	
			console.log("profvis "+p.profvis);
		});
	});

	console.log(profperm === "Public");
	if (profperm === "Public") {
		console.log("went here");
		cb(true);
	} else if (profperm === "Followers Only") {
		console.log("does it also go here");
		db.serialize( function () {
			db.get("select * from isfollowing where username=? and fusername=?", [loggedin,profileowner], function (err, rows) {
				if (err) {
					cb(false);
				} else {
					cb(true);
