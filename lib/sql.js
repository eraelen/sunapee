			...
			if (ncurrentpass === row['password']) {
				...
				if (nnewpass !== '') {
					if ((nnewpass === nconfirmnewpass) && (nnewpass.length > 6)) {
						db.run("update users set password = ? where username = ?", [nnewpass, username]);
					} else {
				...
				if (nname !== '') {
					db.run("update users set name = ? where username = ?", [nname, username]);
					//use of foreign key will update tweets with update name
				}
				
				if (nemail !== '') {
					db.run("update users set email = ? where username = ?", [nemail, username]);
				}
				
				//can be empty
				db.run("update users set location = ? where username = ?", [nlocation, username]);
				db.run("update users set website = ? where username = ?", [nwebsite, username]);
				
				//last change will be to username
				if (nusername !== '') {
					db.run("update users set username = ? where username = ?", [nusername, username]);
					un = nusername;
