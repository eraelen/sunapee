...
<<<<<<< HEAD
		async.each(result.at, function(at,cb1){ db.run('insert into mentions values (?,?)',[id.tweetid,at]); cb1(null);},
=======
		console.log('result '+result.at+' '+result.ht);
		async.each(result.at, function(at,cb1){ console.log('each at'+at); db.run('insert or ignore into mentions values (?,?)',[id.tweetid,at]); cb1(null);},
>>>>>>> retweet working, cleaned up
			function(err){
				async.each(result.ht, 
<<<<<<< HEAD
					function(ht,cb2){ db.run('insert into hashtags values (?,?)',[id.tweetid,ht]); cb2(null);},
=======
					function(ht,cb2){ console.log('each ht'+ht); db.run('insert or ignore into hashtags values (?,?)',[id.tweetid,ht]); cb2(null);},
>>>>>>> retweet working, cleaned up
					function(err) {
						cb(null);
					}
