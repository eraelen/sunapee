...
exports.getTByMention = function(username, number, cb){
	...
	db.all("select * from tweets where msg like '%@ order by date desc"+username+"%'",function(err, u){
...
/*	Returns 5 most recent hashtags currently in the database.
*
*	@return {list} list of hashtags
*/
exports.getTrendingHT = function getTrendingHT(cb) {
	var htList = [];
	db.all("select * from hashtags order by tweetid desc", function(err,ht) {
		for (var i=0; i<5; i++) {
